...
public class ProfileActivity extends AppCompatActivity {
    @Override public void onCreate(Bundle state) { super.onCreate(state); setContentView(R.layout.activity_profile); SessionManager session=new SessionManager(this); TextView info=findViewById(R.id.txtProfileInfo); info.setText(session.getUser().username+"\nVai trò: "+session.getUser().role); PermissionHelper.showIf(findViewById(R.id.btnSettings), PermissionHelper.isAdmin(this)); findViewById(R.id.btnBack).setOnClickListener(v->finish()); findViewById(R.id.btnSettings).setOnClickListener(v->startActivity(new Intent(this, SettingsActivity.class))); findViewById(R.id.btnProfileLogout).setOnClickListener(v->{session.clear();startActivity(new Intent(this, LoginActivity.class));finishAffinity();}); }
}
