...
public class PosActivity extends AppCompatActivity {
    ...
    @Override public void onCreate(Bundle state) { super.onCreate(state); setContentView(R.layout.activity_pos); BottomNavHelper.bind(this); findViewById(R.id.btnBack).setOnClickListener(v -> finish()); productDao=new ProductDao(new DatabaseHelper(this)); cartSummary=findViewById(R.id.txtCartSummary);
        ListView list=findViewById(R.id.listProducts); adapter=new ProductPosAdapter(this); list.setAdapter(adapter);
        ...
        findViewById(R.id.btnCheckout).setOnClickListener(v -> { if(CartManager.get().isEmpty()) { Toast.makeText(this,"Giỏ hàng đang trống",Toast.LENGTH_SHORT).show(); return; } startActivity(new Intent(this, CartActivity.class)); });
        load(""); refreshCart();
    }
    @Override protected void onResume() { super.onResume(); refreshCart(); }
    private void load(String query) { products.clear(); products.addAll(productDao.search(query)); adapter.submit(products); }
